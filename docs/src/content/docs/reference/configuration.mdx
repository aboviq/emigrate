...
* <Link href="/plugins/templates/">Template Plugins</Link> - are used for generating the contents of new migration files. These are only used by the `new` command.
...
:::tip[Did you know?]
The same package can expose multiple plugins, so you can specify the plugin only once and it can be used as both a loader and a template plugin (and storage, in the case of <Link href="/plugins/storage/mysql/">MySQL</Link> for instance).
:::
...
**type:** <code>"{standardPrefixOptions.join('" | "')}" | (name: string, lastMigration?: MigrationMetadata) => string</code>  
**default:** `timestamp`

The type of prefix to use for the new migration file.

```js title="emigrate.config.js" {2-7}
export default {
  new: {
...
```js title="emigrate.config.js" {1,4-6}
...
It's also used for replacing whitespace and characters that are not allowed in filenames in the name This option is only used by the `new` command.
