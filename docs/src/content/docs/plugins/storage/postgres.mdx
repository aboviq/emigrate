...
The PostgreSQL storage plugin uses a PostgreSQL database to store the migration history (*duh*). In the same package you can find the <Link href="/plugins/loaders/postgres/">PostgreSQL Loader Plugin</Link> and the <Link href="/plugins/templates/postgres/">PostgreSQL Template Plugin</Link>.
