...
The MySQL loader plugin transforms `.sql` files into JavaScript functions that Emigrate can use to execute the migrations. In the same package you can find the <Link href="/plugins/templates/mysql/">MySQL Template Plugin</Link> and the <Link href="/plugins/storage/mysql/">MySQL Storage Plugin</Link>.
